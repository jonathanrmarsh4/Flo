...
import { Plus, Activity, TrendingUp, TrendingDown, User, Lightbulb, LogOut, Moon, Sun, X, Sparkles, Upload, FileText, Edit3 } from 'lucide-react';
...
const BIOMARKERS: Record<string, BiomarkerConfig> = {
  ...
  'Fibrinogen': { unit: 'mg/dL', min: 200, max: 400, category: 'Inflammation & Immune Markers' },
  'ANA': { unit: 'titer', min: 0, max: 1, category: 'Inflammation & Immune Markers' },
  'Rheumatoid Factor': { unit: 'IU/mL', min: 0, max: 14, category: 'Inflammation & Immune Markers' },
  
  // Cardiometabolic & Advanced Panels
  'LP-PLA2': { unit: 'ng/mL', min: 0, max: 200, category: 'Cardiometabolic & Advanced Panels' },
  'Galectin-3': { unit: 'ng/mL', min: 0, max: 17.8, category: 'Cardiometabolic & Advanced Panels' },
  'NT-proBNP': { unit: 'pg/mL', min: 0, max: 125, category: 'Cardiometabolic & Advanced Panels' },
  
  // Infectious Disease & General Health
  'CBC with Differential': { unit: 'panel', min: 0, max: 1, category: 'Infectious Disease & General Health' },
  
  // Longevity & Specialized Panels
  'Telomere Length': { unit: 'kb', min: 7.0, max: 12.0, category: 'Longevity & Specialized Panels' },
  '8-OHdG': { unit: 'ng/mL', min: 0, max: 5, category: 'Longevity & Specialized Panels' },
  'MDA': { unit: 'nmol/mL', min: 0, max: 2.5, category: 'Longevity & Specialized Panels' },
  'IL-6': { unit: 'pg/mL', min: 0, max: 5, category: 'Longevity & Specialized Panels' },
  'TNF-α': { unit: 'pg/mL', min: 0, max: 8, category: 'Longevity & Specialized Panels' },
};

const INITIAL_DATA: Reading[] = [
  // Basic Panels
  { id: '1', biomarker: 'Glucose', value: 95, date: '2025-10-15' },
  { id: '2', biomarker: 'Glucose', value: 88, date: '2025-09-12' },
  { id: '3', biomarker: 'Glucose', value: 102, date: '2025-08-08' },
  { id: '4', biomarker: 'Glucose', value: 85, date: '2025-07-05' },
  
  // Lipid & Cardiovascular Health
  { id: '5', biomarker: 'Total Cholesterol', value: 195, date: '2025-10-15' },
  { id: '6', biomarker: 'Total Cholesterol', value: 210, date: '2025-08-10' },
  { id: '7', biomarker: 'Total Cholesterol', value: 185, date: '2025-06-05' },
  { id: '8', biomarker: 'HDL', value: 55, date: '2025-10-15' },
  { id: '9', biomarker: 'LDL', value: 95, date: '2025-10-15' },
  { id: '10', biomarker: 'Triglycerides', value: 120, date: '2025-10-15' },
  
  // Nutritional & Vitamin Status
  { id: '11', biomarker: 'Vitamin D (25-OH)', value: 28, date: '2025-10-15' },
  { id: '12', biomarker: 'Vitamin D (25-OH)', value: 35, date: '2025-08-10' },
  { id: '13', biomarker: 'Vitamin D (25-OH)', value: 42, date: '2025-06-05' },
  { id: '14', biomarker: 'Vitamin B12', value: 450, date: '2025-10-15' },
  { id: '15', biomarker: 'Ferritin', value: 85, date: '2025-10-15' },
  
  // Basic Panels - Blood
  { id: '16', biomarker: 'Hemoglobin', value: 14.5, date: '2025-10-15' },
  { id: '17', biomarker: 'Hemoglobin', value: 13.8, date: '2025-08-10' },
  
  // Liver & Kidney
  { id: '18', biomarker: 'ALT', value: 25, date: '2025-10-15' },
  { id: '19', biomarker: 'Creatinine', value: 1.1, date: '2025-10-15' },
  
  // Hormonal & Endocrine
  { id: '20', biomarker: 'TSH', value: 2.5, date: '2025-10-15' },
  
  // Metabolic & Diabetes
  { id: '21', biomarker: 'HbA1c', value: 5.3, date: '2025-10-15' },
  { id: '22', biomarker: 'HbA1c', value: 5.5, date: '2025-08-10' },
];
...
export default function App() {
  const [readings, setReadings] = useState<Reading[]>(INITIAL_DATA);
  ...
  const [insightsBiomarker, setInsightsBiomarker] = useState<string | null>(null);
  ...
  const [showFullReportScreen, setShowFullReportScreen] = useState(false);
  ...
  const isInRange = (biomarker: string, value: number) => {
    const config = BIOMARKERS[biomarker as keyof typeof BIOMARKERS];
  ...
  return (
    <div className={`min-h-screen pb-20 transition-colors ${
      ...
      {/* Main Content */}
      <main className="px-3 py-3">{/* Changed to remove container and reduce padding */}
        <div className="space-y-3">
          {filteredBiomarkers.map(biomarker => {
            ...
            return (
              <div 
                key={biomarker}
                onClick={() => setInsightsBiomarker(biomarker)}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className={`relative h-full w-full flex flex-col ${
            ...
            <div className={`flex items-center justify-between px-4 py-4 border-b ${
              ...
              <h2 className={`${isDark ? 'text-white' : 'text-gray-900'}`}>Add Test Results</h2>
              <button 
                onClick={() => setShowModal(false)}
                className={`p-2 rounded-lg transition-colors ${
                  isDark ? 'hover:bg-white/10' : 'hover:bg-black/5'
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {addMode === 'manual' ? (
                <form onSubmit={handleAdd} className="space-y-4 max-w-md mx-auto">
                  <div>
                    <label className={`block text-sm mb-2 ${isDark ? 'text-white/70' : 'text-gray-600'}`}>
                      Biomarker
                    </label>
                    <select
                  ...
                  <div>
                    ...
                    {selectedBiomarker && (
                      <p className={`text-xs mt-1.5 ${isDark ? 'text-white/40' : 'text-gray-500'}`}>
                        Optimal range: {BIOMARKERS[selectedBiomarker].min} - {BIOMARKERS[selectedBiomarker].max} {BIOMARKERS[selectedBiomarker].unit}
                      </p>
                    )}
                  </div>
                  ...
                  {selectedBiomarker && (
                    <div>
                      ...
                      <select
                        ...
                        required
                      >
                        {getAvailableUnits(selectedBiomarker).map(unit => (
                          <option key={unit} value={unit}>{unit}</option>
                        ))}
                      </select>
                      {getAvailableUnits(selectedBiomarker).length > 1 && (
                        <p className={`text-xs mt-1.5 ${isDark ? 'text-white/40' : 'text-gray-500'}`}>
                          💡 Select the unit shown on your lab report
                        </p>
                      )}
                    </div>
                  )}
                ...
                <div className="max-w-md mx-auto">
                  ...
                  >
                    <Upload className={`w-16 h-16 mx-auto mb-4 ${isDark ? 'text-white/40' : 'text-gray-400'}`} />
                    <h3 className={`mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      Upload Lab Results
                    </h3>
                    <p className={`text-sm mb-4 ${isDark ? 'text-white/60' : 'text-gray-600'}`}>
                      Drag and drop your lab results here, or click to browse
                    </p>
                    <input 
                      type="file" 
                      id="file-upload"
                    ...
                    <p className={`text-xs mt-4 ${isDark ? 'text-white/40' : 'text-gray-500'}`}>
                  ...
                  <div className={`mt-6 p-4 rounded-xl ${
                    ...
                    <div className="flex gap-3">
                      ...
                      <div>
                        <h4 className={`text-sm mb-1 ${isDark ? 'text-blue-300' : 'text-blue-900'}`}>
                          AI-Powered Extraction
                        </h4>
                        <p className={`text-xs ${isDark ? 'text-blue-200' : 'text-blue-800'}`}>
                          Our AI will automatically extract biomarker values from your lab reports. You'll be able to review and confirm the data before saving.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className={`sticky top-0 z-10 backdrop-blur-xl border-b px-5 py-4 ${
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className={`${isDark ? 'text-white' : 'text-gray-900'}`}>{insightsBiomarker} Insights</h2>
                </div>
                <button 
                  onClick={() => setInsightsBiomarker(null)}
                  className={`p-2 rounded-lg transition-colors ${
            ...
            <div className="overflow-y-auto p-5 space-y-4" style={{ maxHeight: 'calc(85vh - 70px)' }}>
              {BIOMARKER_INSIGHTS[insightsBiomarker] ? (
                <>
                  ...
                  <div className={`rounded-2xl p-4 ${
                    ...
                    <ul className="space-y-1.5">
                      {BIOMARKER_INSIGHTS[insightsBiomarker].lifestyle.map((item, index) => (
                        <li key={index} className={`text-xs ${isDark ? 'text-blue-100' : 'text-blue-800'}`}>
                          • {item}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className={`rounded-2xl p-4 ${
                    ...
                    <ul className="space-y-1.5">
                      {BIOMARKER_INSIGHTS[insightsBiomarker].nutrition.map((item, index) => (
                        <li key={index} className={`text-xs ${isDark ? 'text-green-100' : 'text-green-800'}`}>
                          • {item}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className={`rounded-2xl p-4 ${
                    ...
                    <ul className="space-y-1.5">
                      {BIOMARKER_INSIGHTS[insightsBiomarker].supplementation.map((item, index) => (
                        <li key={index} className={`text-xs ${isDark ? 'text-purple-100' : 'text-purple-800'}`}>
                          • {item}
                        </li>
                      ))}
                    </ul>
                  ...
                  {/* Medical Referral */}
                  {BIOMARKER_INSIGHTS[insightsBiomarker].medicalReferral && (
                    <div className={`rounded-2xl p-4 ${
                      isDark ? 'bg-red-500/10 border border-red-500/20' : 'bg-red-50 border border-red-200'
                    }`}>
                      <h3 className={`text-sm mb-2 flex items-center gap-2 ${isDark ? 'text-red-300' : 'text-red-900'}`}>
                        <TrendingDown className="w-4 h-4" />
                        Medical Referral
                      </h3>
                      <p className={`text-xs ${isDark ? 'text-red-100' : 'text-red-800'}`}>
                        {BIOMARKER_INSIGHTS[insightsBiomarker].medicalReferral}
                      </p>
                    </div>
                  )}
                  ...
                  <div className={`text-[10px] p-3 rounded-xl ${
                    ...
                    ⚠️ These insights are for educational purposes only and should not replace professional medical advice. Always consult with qualified healthcare providers before making changes to your health regimen.
                  </div>
                </>
              ) : (
                <div className={`rounded-2xl p-6 text-center ${
                  ...
                  <Sparkles className={`w-12 h-12 mx-auto mb-3 ${isDark ? 'text-purple-400' : 'text-purple-600'}`} />
                  <h3 className={`mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    AI Insights Coming Soon
                  </h3>
                  <p className={`text-sm ${isDark ? 'text-white/60' : 'text-gray-600'}`}>
                    Detailed insights for {insightsBiomarker} will be available in a future update. In the meantime, consult with your healthcare provider about your results.
                  </p>
                </div>
              )}
            </div>
      ...
      {/* Bottom Navigation */}
      <nav className={`fixed bottom-0 left-0 right-0 z-50 backdrop-blur-xl border-t transition-colors ${
        isDark ? 'bg-white/5 border-white/10' : 'bg-white/70 border-black/10'
      }`}>
        <div className="flex items-center justify-around px-4 py-3">
          ...
          >
            <User className={`w-5 h-5 ${isDark ? 'text-white/70' : 'text-gray-600'}`} />
            <span className={`text-[10px] ${isDark ? 'text-white/70' : 'text-gray-600'}`}>Profile</span>
          </button>
          
          <button 
            onClick={() => setShowModal(true)}
            className="flex flex-col items-center gap-1 px-4 py-2 -mt-2"
          >
          ...
          <button 
            onClick={() => setShowInsightsScreen(true)}
            className="flex flex-col items-center gap-1 px-4 py-2"
          >
            <Lightbulb className={`w-5 h-5 ${isDark ? 'text-white/70' : 'text-gray-600'}`} />
            <span className={`text-[10px] ${isDark ? 'text-white/70' : 'text-gray-600'}`}>Insights</span>
          </button>
        </div>
      </nav>
