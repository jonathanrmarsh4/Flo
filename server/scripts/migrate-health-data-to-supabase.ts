  ...
  for (const night of nights) {
    ...
    try {
      const { error } = await supabase.from('sleep_nights').upsert({
        ...
        bedtime: night.bedtime?.toISOString(),
        wake_time: night.wakeTime?.toISOString(),
        time_in_bed_hours: night.timeInBedHours,
        time_asleep_hours: night.timeAsleepHours,
        sleep_efficiency: night.sleepEfficiency,
        awakenings: night.awakenings,
        rem_hours: night.remHours,
        deep_hours: night.deepHours,
        core_hours: night.coreHours,
        awake_hours: night.awakeHours,
        sleep_score: night.sleepScore,
        hr_lowest: night.hrLowest,
        hr_average: night.hrAverage,
        hrv_average: night.hrvAverage,
        respiratory_rate: night.respiratoryRate,
        spo2_average: night.spo2Average,
        spo2_lowest: night.spo2Lowest,
        source_name: night.sourceName,
        source_bundle_id: night.sourceBundleId,
