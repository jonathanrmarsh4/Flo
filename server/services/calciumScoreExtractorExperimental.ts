...
// Same schema as the production extractor
...
async function extractCalciumScoreWithGPT(pdfText: string, modelName: string): Promise<CalciumScoreExtraction> {
  const systemPrompt = `You are a precise coronary calcium score report analyzer. Your job is to extract structured data from cardiac CT imaging reports, even from poor quality or difficult-to-read PDFs.
...
11. Look for common OCR errors and correct them (e.g., "O" vs "0", "l" vs "1")
...
- Flag low confidence in the quality.confidence field`;
  ...
  const userPrompt = `Extract coronary calcium score data from this cardiac CT report. This may be a poor quality PDF with OCR issues or unclear formatting. Use your best judgment to extract accurate data:
