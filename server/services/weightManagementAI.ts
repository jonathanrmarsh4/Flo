...
export async function generateWeightAnalysis(userId: string): Promise<WeightAIAnalysis> {
  ...
  try {
    ...
    // Note: Removed responseMimeType and responseSchema to avoid Gemini structured output
    // truncation issues with complex nested schemas. The extractJsonFromResponse() parser
    // handles raw JSON extraction reliably.
