...
export async function extractDexaScanExperimental(
  ...
  try {
    ...
    console.log(`[DEXA Experimental] Starting OCR + ${modelName} extraction...`);
    ...
    console.log(`[DEXA Experimental] Extracted ${pdfText.length} chars, sending to ${modelName}...`);
    const extraction = await extractDexaScanWithGPT(pdfText, modelName);
    
    extraction.source_document.filename = filename;

    console.log(`[DEXA Experimental] Extraction successful with ${modelName}`);
    return {
      success: true,
      data: extraction,
      pdfText,
    ...
    console.error("[DEXA Experimental] Error:", error);
    const modelName = options?.model || "gpt-5";
    return {
      success: false,
