/**
 * Apple Root Certificates for App Store JWS Verification
 * 
 * These certificates are required by @apple/app-store-server-library
 * to verify the signature of StoreKit 2 transactions and App Store
 * Server Notifications.
 * 
 * Source: https://www.apple.com/certificateauthority/
 * 
 * Certificates included:
 * - Apple Root CA - G3 (EC, valid until 2039)
 * - Apple Root CA - G2 (RSA 4096, valid until 2039)
 * - Apple Inc. Root Certificate (RSA 2048, valid until 2035)
 */

// Apple Root CA - G3 (EC P-384)
// Valid: 2014-04-30 to 2039-04-30
const APPLE_ROOT_CA_G3_BASE64 = '';

// Apple Root CA - G2 (RSA 4096)
// Valid: 2014-04-30 to 2039-04-30
const APPLE_ROOT_CA_G2_BASE64 = '';

// Apple Inc. Root Certificate (RSA 2048)
// Valid: 2006-04-25 to 2035-02-09
const APPLE_INC_ROOT_BASE64 = '';

/**
 * Get Apple Root Certificates as Buffer array
 * Required by SignedDataVerifier from @apple/app-store-server-library
 */
export function getAppleRootCertificates(): Buffer[] {
  return [
    Buffer.from(APPLE_ROOT_CA_G3_BASE64, 'base64'),
    Buffer.from(APPLE_ROOT_CA_G2_BASE64, 'base64'),
    Buffer.from(APPLE_INC_ROOT_BASE64, 'base64'),
  ];
}

export const APPLE_ROOT_CA_G3 = Buffer.from(APPLE_ROOT_CA_G3_BASE64, 'base64');
export const APPLE_ROOT_CA_G2 = Buffer.from(APPLE_ROOT_CA_G2_BASE64, 'base64');
export const APPLE_INC_ROOT = Buffer.from(APPLE_INC_ROOT_BASE64, 'base64');
