...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Heart & Metabolic Health Detail Analysis
  app.get("/api/heart-metabolic/analysis", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { calculateDashboardScores } = await import("./services/scoreCalculator");
      const { generateHeartMetabolicAnalysis } = await import("./services/healthAnalysis");

      // Get scores and detailed component breakdown
      const scores = await calculateDashboardScores(userId);
      
      if (!scores.cardiometabolic) {
        return res.status(404).json({ 
          error: "No cardiometabolic data available. Please upload blood work and/or diagnostic studies." 
        });
      }

      // Generate AI-powered recommendations based on component scores
      const analysis = await generateHeartMetabolicAnalysis(userId, scores.details.cardiometabolicDetails);

      res.json({
        overallScore: scores.cardiometabolic,
        riskBand: scores.details.cardiometabolicDetails.riskBand,
        components: {
          lipids: scores.details.cardiometabolicDetails.lipidsScore,
          glycemic: scores.details.cardiometabolicDetails.glycemicScore,
          bloodPressure: scores.details.cardiometabolicDetails.bloodPressureScore,
          calciumScore: scores.details.cardiometabolicDetails.cacScore,
        },
        recommendations: analysis.recommendations,
        biomarkerDetails: analysis.biomarkerDetails,
      });
    } catch (error) {
      console.error("Error generating heart & metabolic analysis:", error);
      res.status(500).json({ error: "Failed to generate analysis" });
    }
  });

  // Body Composition Detail Analysis
  app.get("/api/body-composition/analysis", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { calculateDashboardScores } = await import("./services/scoreCalculator");
      const { generateBodyCompositionAnalysis } = await import("./services/healthAnalysis");

      // Get scores and detailed component breakdown
      const scores = await calculateDashboardScores(userId);
      
      if (!scores.bodyComposition) {
        return res.status(404).json({ 
          error: "No body composition data available. Please upload a DEXA scan." 
        });
      }

      // Generate AI-powered recommendations based on component scores
      const analysis = await generateBodyCompositionAnalysis(userId, scores.details.bodyCompositionDetails);

      res.json({
        overallScore: scores.bodyComposition,
        components: {
          bodyFatPercent: scores.details.bodyCompositionDetails.fatPercent,
          leanMassPercent: scores.details.bodyCompositionDetails.leanPercent,
          visceralFatArea: scores.details.bodyCompositionDetails.visceralFatArea,
          visceralFatScore: scores.details.bodyCompositionDetails.visceralFatScore,
          boneHealth: scores.details.bodyCompositionDetails.boneHealth,
          boneTScore: scores.details.bodyCompositionDetails.boneTScore,
        },
        recommendations: analysis.recommendations,
        dexaDetails: analysis.dexaDetails,
      });
    } catch (error) {
      console.error("Error generating body composition analysis:", error);
      res.status(500).json({ error: "Failed to generate analysis" });
    }
  });
