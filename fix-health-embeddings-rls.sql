...
CREATE POLICY "Users can view own embeddings" ON public.health_embeddings FOR SELECT TO authenticated USING ((select auth.uid())::text = user_id);

CREATE POLICY "Users can insert own embeddings" ON public.health_embeddings FOR INSERT TO authenticated WITH CHECK ((select auth.uid())::text = user_id);

CREATE POLICY "Users can update own embeddings" ON public.health_embeddings FOR UPDATE TO authenticated USING ((select auth.uid())::text = user_id) WITH CHECK ((select auth.uid())::text = user_id);

CREATE POLICY "Users can delete own embeddings" ON public.health_embeddings FOR DELETE TO authenticated USING ((select auth.uid())::text = user_id);
