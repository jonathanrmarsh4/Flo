...
- **ClickHouse ML Correlation Engine:** High-performance analytics for anomaly detection, predictive insights, and long-term pattern recognition, with full history sync and 90-day baseline + Z-score anomaly detection. **Sleep Baseline Filtering (Dec 2025):** Baseline queries now filter out naps and partial sleep sessions (time_in_bed < 4h, sleep_duration < 3h) to prevent artificially low baselines. **Temperature Alert Fixes (Dec 2025):** Temperature deviation metrics (wrist_temperature_deviation, skin_temp_deviation_c, etc.) now use ABSOLUTE thresholds (0.4°C to alert, 0.7°C high severity) instead of percentage-based thresholds. Physiological bounds validation rejects values >2.5°C as sensor errors. Corroborating vital signs require meaningful deviation (HR +5%, HRV -10%, RR +8%, SpO2 -1.5%) to validate temperature alerts. Corroborating vitals are stored in `relatedMetrics` and explicitly mentioned in AI-generated insight text.
...
- **Dexcom CGM Integration:** Direct OAuth2 integration with Dexcom API for continuous glucose monitoring, automatic 5-minute data sync, and real-time glucose dashboard tile. **HealthKit Glucose Fallback (Dec 2025):** The `/api/cgm/data` endpoint now supports dual data sources - it first checks for Dexcom readings, then falls back to HealthKit glucose samples (`HKQuantityTypeIdentifierBloodGlucose`) from the `healthkit_samples` table. CGMScreen displays data from either source with appropriate source labeling ("HealthKit" vs "Dexcom"). Trend calculation for HealthKit data is computed from the last 2 readings when within 15 minutes of each other.
...
- **Apple App Store AI Compliance (Dec 2025):** Full compliance system for sharing health data with third-party AI services:
  - **AIConsentScreen** (`client/src/components/onboarding/AIConsentScreen.tsx`): Onboarding step after security setup that explains AI data sharing with Google AI (Gemini), OpenAI, and ElevenLabs. Features benefits-first messaging, privacy assurances ("data anonymized", "stored separately"), and enable/skip paths.
  - **requireAIConsent Middleware** (`server/middleware/planEnforcement.ts`): Gates all AI endpoints (Oracle chat, voice relay, insights, briefings) and returns 403 `AI_CONSENT_REQUIRED` if user hasn't consented.
  - **Settings Privacy Toggle** (`ProfileScreen.tsx`): AI Features toggle in Data & Privacy section allows users to revoke/grant consent at any time.
  - **AI Provider Attribution**: "Powered by Google AI" footer in WhyModal, "Powered by Google AI & ElevenLabs" in VoiceChatScreen for transparency.
  - **Privacy Policy Updates** (`PrivacyPolicyScreen.tsx`): Section 4.2 enhanced with explicit AI vendor disclosure, data anonymization explanation, and consent controls.
  - **Version Tracking**: `CURRENT_AI_CONSENT_VERSION` in routes.ts enables re-prompting when AI policy changes. GET `/api/user/ai-consent` returns `needsReconsent: true` when user's stored version doesn't match current version.
  - **Database Fields**: `aiConsentGranted` (boolean), `aiConsentDate` (timestamp), `aiConsentVersion` (varchar) in users table.
