...
- **ML Causality Engine (Hybrid ML+AI Architecture):** Identifies WHY health metrics change by analyzing behavior patterns across full history (years if available). **Data Flow:** ClickHouse computes baselines → BehaviorAttribution syncs factors → ML ranks causes with deviation percentages → Gemini formats narratives → Database stores `likely_causes` array. Key components:
  - **BehaviorAttributionEngine:** Syncs daily behavior factors (nutrition, workouts, recovery, supplements, life events, environment, CGM data) to ClickHouse `daily_behavior_factors` table with 30-day baseline calculations for each factor. Queries `health_metrics` joined with `metric_baselines` for deviations. Includes date validation for time_value fields to prevent "Invalid time value" errors.
