...
export function useGeminiLiveVoice(options: UseGeminiLiveVoiceOptions = {}) {
  ...
  // Play queued audio buffers with precise timing to prevent gaps/crackle on iOS
  ...
  const playNextAudio = useCallback(() => {
    ...
    // Schedule playback with precise timing to prevent gaps on iOS
    ...
    // Schedule next buffer with tiny overlap (2ms) to prevent pops between chunks
    const overlap = 0.002;
    ...
    source.start(startTime); // Schedule at precise time
  }, []);
