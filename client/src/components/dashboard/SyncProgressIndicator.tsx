...
export function SyncProgressIndicator({ isDark, hasDashboardData }: SyncProgressIndicatorProps) {
  ...
  // Established users with existing data don't need the sync banner
  if (hasDashboardData) return null;
  if (isLoading) return null;
  if (error || !syncStatus) return null;

  // Hide banner once recent metrics are ready (tiles can display)
  if (syncStatus.recentMetricsReady) {
    if (syncStatus.healthkitSyncing) {
      return (
        <div
  ...
  if (syncStatus.isReady && syncStatus.healthkitRecords > 0) return null;

  const isSyncing = syncStatus.healthkitSyncing;
          ...
          {isSyncing && (
            <div className="mt-2">
              ...
              <p className={`text-xs mt-1 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                {syncStatus.healthkitRecords > 0 && (
                  <>{syncStatus.healthkitRecords.toLocaleString()} health records synced</>
                )}
              </p>
            </div>
          )}
