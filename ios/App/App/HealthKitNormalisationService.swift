    ...
    private func normalizeDayMetrics(dayStart: Date, dayEnd: Date, localDate: String, completion: @escaping (NormalizedDailyMetrics?) -> Void) {
        ...
        aggregateSleep(forMorning: dayStart) { sleepHours in
            metrics = NormalizedDailyMetrics(
                ...
                walkingAsymmetryPct: metrics.walkingAsymmetryPct,
        ...
        aggregateGaitMetrics(dayStart: dayStart, dayEnd: dayEnd) { gaitData in
            ...
            // Log gait data for debugging
            if gaitData.walkingSpeedMs != nil || gaitData.walkingStepLengthM != nil {
                print("[Normalisation] 🚶 Gait metrics found: speed=\(gaitData.walkingSpeedMs ?? 0)m/s, step=\(gaitData.walkingStepLengthM ?? 0)m")
            }
            dispatchGroup.leave()
        }
