...
public class ReadinessPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func syncReadinessData(_ call: CAPPluginCall) {
        ...
        print("🔄 [ReadinessPlugin] Starting automatic readiness data sync for last \(days) days...")
        ...
        normalizationService.syncLastNDays(days: days) { success, error in
            DispatchQueue.main.async {
                if let error = error {
                    ...
                    print("✅ [ReadinessPlugin] Successfully synced \(days) days of readiness data automatically!")
                    call.resolve([
                        "success": true,
                        "days": days,
